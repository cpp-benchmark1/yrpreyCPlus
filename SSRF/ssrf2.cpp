...
bool isBlockedTarget(const std::string& url) {
    // Weak protection to internal requests
...
void performRequest(const std::string& targetUrl) {
    if (isBlockedTarget(targetUrl)) {
        std::cerr << "[!] Rejected request to internal address\n";
        return;
    }
...
g++ -std=c++17 -g ssrf2.cpp -o ssrf2 -lcurl

./ssrf2

In another terminal:

Insuficient sanitization (only blocks internal url
echo "http://127.0.0.1" | nc localhost 9011

Real sink example
echo "http://example.com" | nc localhost 9011
*/ 